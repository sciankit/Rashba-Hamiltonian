...
def E_cal(K_y, K_z):
    ...
    E2 = (
        ...
        / 1.6e-19
        * 1e20
        * ((h_bar**2 * K_y**2 / 2 / effM_y) + (h_bar**2 * K_z**2 / 2 / effM_z))
    )
...
for i in range(len(X)):
    if X[i] < 0:
        Y1[i], Y2[i] = E_cal(0, -X[i] * 2 * math.pi / clatticevector)
    else:
        Y1[i], Y2[i] = E_cal(X[i] * 2 * math.pi / blatticevector, 0)

fig = plt.figure()
ax = fig.add_subplot(111)
plt.plot(X, Y1, c="red")
plt.plot(X, Y2, c="red")
plt.axhline(0, color="black", linewidth=0.5), plt.axvline(
...
np.savetxt(
    "Output2.dat",
    np.column_stack(([x + 0.5 for x in X], Y1, Y2)),
    fmt="%f",
    delimiter="\t",
    header="\t\t\t".join(titles),
)
